 ...
 */
abstract class AbstractDefaultPage
{
	//---------------------------------------------------------------------------------------------
	// ~ Static variables
	//---------------------------------------------------------------------------------------------
	...
	public function __construct($file, $options=null)
	{
		...
		if (isset($this->option_name)) {
			...
			if (function_exists('settings_errors')) add_action('admin_notices', 'settings_errors');
		}

		add_action('admin_menu', array($this, 'page_init'), $this->args['admin_action_priority']);
		add_filter('contextual_help', array($this, '_contextual_help'), 10, 2);

		if ($this->args['action_link']) add_filter('plugin_action_links_' . plugin_basename($file), array($this, '_action_link'));
	}
...
//  ____________REGISTRATION COMPONENT____________
	...
	public static function register($file, $options = null)
	{
		$class = \get_called_class();
		if (isset(self::$registered[$class])) return false;
	...
	{
		foreach (self::$registered as $class => $args) new $class($args[0], $args[1]);
	}
...
//  ____________INTERNAL METHODS____________
	...
	{
		if (empty($this->args['page_title'])) trigger_error('Page title cannot be empty', E_USER_WARNING);
