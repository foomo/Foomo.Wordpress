...
class Forms
{
	...
	static function input($args, $formdata = false)
	{
		...
		if (!empty($formdata)) {
			$form = new \Foomo\Wordpress\Admin\Form($formdata);
...
// ____________PRIVATE METHODS____________
	...
	private static function _select($args)
	{
		...
		foreach ($options as $option) {
			...
			$opts .= \Foomo\Wordpress\View::html('option', $title, compact('value', 'selected'));
		}

		$extra['name'] = self::$cur_name;

		$input = \Foomo\Wordpress\View::html('select', $opts, $extra);
	...
	private static function _checkbox($args)
	{
		...
		if (is_null($desc) && !is_bool($value)) $desc = str_replace('[]', '', $value);
	...
	private static function _input_gen($args)
	{
		...
		if ('textarea' == $type) {
			$input = \Foomo\Wordpress\View::html('textarea', esc_textarea($value), $extra);
		} else {
			$extra['value'] = $value;
			$extra['type'] = $type;
			$input = \Foomo\Wordpress\View::html('input', null, $extra);
		}
	...
	private static function add_label($input, $desc, $desc_pos)
	{
		...
		if (false === strpos($desc, self::token)) {
			switch ($desc_pos) {
				case 'before': $label = $desc . ' ' . self::token;
					break;
		...
		if (empty($desc))
			...
			$output = \Foomo\Wordpress\View::html('label', $label);
